...
namespace finance {
	void account_reclaim(MYSQL* conn) {
		...
		MYSQL_ROW row;
		int id;                                          //将数据库中的数据读取出来放入基础的变量类型中，然后使用这些变量去构造一个写的对象并放到向量中
		std::string name;        
		std::string holder;
		std::string number;
		double money;
		std::string delete_query;
		while ((row = mysql_fetch_row(result)) != NULL) {        //将从数据库中读取的书据放入变量中
			id = atoi(row[0]);
			name = row[1];
			holder = row[2];
			number = row[3];
			money = atof(row[4]);
			Account* ptr = new Account(name, holder, number, money);   //创建对象并放入向量中，方便后面在程序中进行操作
	...
	void ledger_reclaim(MYSQL* conn) {
		...
		while ((row = mysql_fetch_row(result)) != NULL) {  //数据的反序列化
			id = atoi(row[0]);
			date1 = atoi(row[1]);
			month = date1 % 100;
			...
			Book.push_back(ledger_ptr);     //将expense函数放在create_ledger中，这样避免了账目信息读取时反复修改账户状态
	...
	void ledger_storage(MYSQL* conn) {
		...
		double transaction_amount;
		while (it != Book.end()) {               //账目信息的序列化（拆分对象为基本信息）
			date1 = (*it)->date.code1();
			date2 = (*it)->date.code2();
			...
			mysql_query(conn, insert_query.c_str());
			delete (*it);
			it = Book.erase(it);    //动态内存管理，释放之前分配的动态内存
		}
